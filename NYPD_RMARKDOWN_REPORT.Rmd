---
title: "NYPD Analysis"
author: "TCJ"
date: "`r Sys.Date()`"
output: pdf_document
---

# NYPD Analysis


## For this project I have decided to do the analysis of the shooting with the main vocus on the victims information. My intention with this report is to create awereness amongst people who fit the demographics of the main victims, so they can try to avoid walking in certain neighboorhoods and during certain times of the day.

## During my analysis, I noticed that the vast majority of the victms are black men. The women victims of the shootings represent a very small part of it, which is very suspicious and I wasn't able to identify why they only represent such a small portion of the cases. My two main guesses are: 1) while filing the report, men were present with woman and the officer decided to file the case with the man as the main figure or 2) women tend to be more careful than men, thus avoiding situations such as bar fights. This, of course, is a sign of bias that I wasn't able to mitigate due to lack of data.

## Additionally, further analysis to identify the reasons as for why the number of shootings drops so drastically at 5am and increases at 2pm, is required to understand the data.


```{r setup_data, include=TRUE, warning=FALSE,  message=FALSE}

#Importing the libraries needed
library(dplyr)
library(lubridate)
library(tidyverse)
library(tidyr)

#Importing the data from the city of New Year website data
nypd <- read_csv(url("https://data.cityofnewyork.us/api/views/833y-fsy8/rows.csv"))

# Correcting time and date type
nypd <- nypd %>% mutate(OCCUR_DATE = mdy(OCCUR_DATE),
                        HOUR = hour(hms(OCCUR_TIME)))

# Dropping columns not used for analysis
nypd <- nypd %>% 
  select(-c(PRECINCT, X_COORD_CD, Y_COORD_CD, Latitude, Longitude, Lon_Lat, 
            LOC_CLASSFCTN_DESC, INCIDENT_KEY, JURISDICTION_CODE))
```

```{r setup, include=TRUE, warning=FALSE,  message=FALSE}
# Grouping by Borough, Date, and Victim's Sex to get count of cases
grouped <- nypd %>% group_by(BORO, OCCUR_DATE, VIC_SEX) %>% summarise(total_count = n())

# Plotting
grouped %>% 
  count(OCCUR_DATE, VIC_SEX, BORO) %>% 
  ggplot(aes(y = n, x = BORO, fill = interaction(VIC_SEX))) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = c("lightpink", "lightblue", "red")) +
  labs(y = "# of Shootings", x = "Borough", fill = "Victim's Sex") +
  theme_minimal() +
  theme(panel.grid = element_blank(), 
        panel.border = element_blank(),  
        axis.line.y = element_blank(),
        axis.line.x = element_line())
```

```

```


## We can see that Brooklyn and Bronx are the borough with the most reported shootings bias: shooting should be reduced per population to mitigate possible bias, as the higher population the more shooting is likely to happen. Additionally, we noticed that the vast majority of the victims are male.

```

```

# Now let's find out the demographics of the victims.


```{r, warning=FALSE, message=FALSE}
victims = nypd %>% 
  group_by(VIC_AGE_GROUP,VIC_RACE, VIC_SEX, STATISTICAL_MURDER_FLAG)%>%
  summarise(total_count = n())

#dropping the datapoint where there seems to have a typo in the age group, saying 1022

victims <- victims %>% filter(VIC_AGE_GROUP !=1022)

# Calculate the percentage of occurrences for each combination of VIC_RACE and 
#VIC_AGE_GROUP
victims <- victims %>%
  mutate(total_vict = sum(victims$total_count))%>%
  mutate(percentage = total_count/total_vict *100)

#aggregating the percentage 
vic_dem <- aggregate(victims$percentage,
                     by=list(Category=victims$VIC_RACE, victims$VIC_SEX), FUN=sum)

#Plotting the results
vic_dem %>% 
  count(Category, Group.2, x) %>% 
  ggplot(aes(y = vic_dem$x, x = vic_dem$Category, fill = interaction(vic_dem$Group.2))) +
  geom_bar(stat = "identity", position = "stack")+
  scale_fill_manual(values = c("lightpink","lightblue","red"))+
  labs(y = "% of Cases", x = "Victim's Race", fill = "Victim's Sex")+
  theme_minimal()+
  theme(panel.grid = element_blank(),
        panel.border = element_blank(),
        axis.line.y = element_blank(),
        axis.line.x = element_line(),
        axis.text.x = element_text(angle = 35, hjust = 1))


```


## As we can see, the vast majority of the victims are black, followed by White Hispanic and then Hispanic. 

# Cases and Deaths by Age Group

## Let try to explore the number of cases by age group and the see percentage of deaths occuring by each group to see what age groups are more likely to get involved in shootings and also know which ones are have the highest death rate.

```{r, warning=FALSE, message=FALSE}


# Aggregating the percentage
victims <- nypd %>%
  group_by(VIC_AGE_GROUP,VIC_RACE, VIC_SEX, STATISTICAL_MURDER_FLAG)%>%
  summarise(total_count = n())

#dropping the datapoint where there seems to have a typo in the age group, saying 1022

victims <- victims %>% filter(VIC_AGE_GROUP !=1022)

# Calculate the percentage of occurrences for each combination of VIC_RACE and VIC_AGE_GROUP

#calculating percentage 

ind_sum <- aggregate(victims$total_count, by=list(victims$VIC_AGE_GROUP), FUN=sum)

total_dem <-aggregate(victims$total_count,
                      by=list(Category=victims$VIC_AGE_GROUP, 
                              victims$STATISTICAL_MURDER_FLAG), FUN=sum) %>%
  mutate(total_dem_count = sum(x)) %>%
  mutate(percent_dem = x / ind_sum$x * 100)

total_dem <-total_dem%>%
  mutate(Group.2 = case_when(
    Group.2 == TRUE ~ "Death reported",
    Group.2 == FALSE ~ "No Death",
    TRUE ~ as.character(Group.2)  # Handle other cases if any
  ))

# Plotting the results
ggplot(total_dem, aes(x = Category, y = x, fill = Group.2)) +
  geom_bar(stat = "identity", position = position_dodge(width = .9)) +
  scale_fill_manual(values = c("No Death" = "#03BFC4", "Death reported" = "#F7766D")) +
  geom_text(aes(label = sprintf("%.1f%%", percent_dem),
                y = x / 2), 
            position = position_dodge(width = .9),
            size = 3) +
  labs(y = "# of Cases", x = "Age Group", fill = "Death Stats") +
  theme_minimal() +
  theme(panel.grid = element_blank(),  
        panel.border = element_blank(), 
        axis.line = element_line())

```


## As we can see by the graph people from age 18 and 44 are more likely to get involved in the shootings, but the highest deaths occur between the older groupsm 45+.

# Now let's see what part of the day most of the cases happened.



```{r, message=FALSE}

#Getting the total count by hour for each borough
period_case <-nypd %>% group_by(HOUR,BORO) %>% summarise(total_count = n())

# Aggregate the total count by HOUR and BORO
time_case <- aggregate(total_count ~ HOUR + BORO, data = period_case, FUN = sum)

# Create hour labels in 12-hour format with "AM" and "PM" indicators
time_case$hour_label <- ifelse(time_case$HOUR < 12, paste0(time_case$HOUR, "AM"), 
                               ifelse(time_case$HOUR == 12, "12PM",
                                      paste0(time_case$HOUR - 12, "PM")))

#Plotting the results
time_case %>% 
  count(BORO, HOUR, total_count) %>% 
  ggplot(aes(y = total_count, x = HOUR, fill = interaction(BORO))) +
  geom_bar(stat = "identity", position = "stack") +
  labs(y = "# of cases", x = "Time of the Day", fill = "Borough") +
  scale_x_continuous(breaks = unique(time_case$HOUR), 
                     labels = unique(time_case$hour_label)) +
  theme_minimal() +
  theme(panel.grid = element_blank(),  
        panel.border = element_blank(),  
        axis.line.y = element_line(),   
        axis.line.x = element_line(),  
    axis.text.x = element_text(angle = 35, hjust = 1))
```


## Our analysis shows that the majority of the shootings happened between 6pm and 5am, with a significant increase after 2pm. The peak of the number of shootings happened between 10 and 11pm and  started to slowly decrease from that until 5am, when it dropped significantly decrease. 


## Further analysis to identify the reasons as for why the number of shootings drops so drastically at 5am and increases at 2pm, is required to understand the data. I could have incremented this analysis by diving deeper into the locations and conditions where the shootings happened, however, I found to be challenging since more than half of the data doesn't not contain the location that the shooting happened ("LOCATION_DESC" or even "LOC_CLASSFCTN_DESC"), thus not enough data was provide to support the analysis. 




# Linear Model

## Let's try to fit a model with the prediction of deaths compared to the total number of cases.

```{r, message=FALSE}


#extracting the month and year of the dates

nypd$Month <- format(nypd$OCCUR_DATE, "%Y-%m")

#creating the dataset with the number of cases for each death toll
monthly_cases <- nypd %>%
  group_by(Month,STATISTICAL_MURDER_FLAG) %>%
  summarise(total_cases = n())

#Pivoting the results, so we can fit it to the model in a better way
separated_cases <- monthly_cases %>%
  pivot_wider(names_from = STATISTICAL_MURDER_FLAG, values_from = total_cases)
names(separated_cases)[2:3] <- c("death", "non_death")
#getting the total of cases, so we can better fit the model
separated_cases$total_cases <- separated_cases$death + separated_cases$non_death

# Fit a linear model to predict the total number of cases
lm_model <- lm(death ~ total_cases, data = separated_cases)

plot_lm <- plot(lm_model)
plot_lm[2]


```
